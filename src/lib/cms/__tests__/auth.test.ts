...
describe("CMS session tokens", () => {
  ...
  it("round-trips a valid token", async () => {
    ...
    expect(session).toEqual(expect.objectContaining({ username: "admin", mustChangePassword: false }));
  });

  it("round-trips a token that requires a password change", async () => {
    const token = await createSessionToken("linda", true);
    const session = await verifySessionToken(token);
    expect(session).toEqual(expect.objectContaining({ username: "linda", mustChangePassword: true }));
  });
