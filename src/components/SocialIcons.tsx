    ...
    <div className={className}>
      {LINKS.map((social) => (
        <a
          ...
          className="flex h-9 w-9 items-center justify-center rounded-full border border-border transition-colors hover:border-accent hover:text-accent-text"
          style={social.color ? { color: social.color, borderColor: social.color } : undefined}
        >
          <svg viewBox="0 0 24 24" fill="currentColor" className={social.color ? "h-5 w-5" : "h-4 w-4 text-muted"}>
            <path d={social.path} />
          </svg>
        </a>
      ))}
